...
registerSDPService uuid info rfcommChannel = do
    ...
    allocaBytes {#sizeof sdp_profile_desc_t #} $ \profile -> do
        ...
        throwErrnoIfNegative_ "sdp_set_service_classes" $ c_sdp_set_service_classes record svcClassList
        ...
        throwErrnoIfNegative_ "sdp_set_profile_descs" $ c_sdp_set_profile_descs record profileList
        
        _ <- c_sdp_uuid16_create rootUuid PublicBrowseGroup
        rootList <- c_sdp_list_append nullPtr rootUuid
        throwErrnoIfNegative_ "sdp_set_browse_groups" $ c_sdp_set_browse_groups record rootList
        ...
        throwErrnoIfNegative_ "sdp_set_access_protos" $ c_sdp_set_access_protos record accessProtoList
        
        case info of
             (SDPAttributes sn d pn) -> c_sdp_set_info_attr record sn d pn
             SDPNoInfo               -> return ()
        
        session <- throwErrnoIfNull "sdp_connect" $ c_sdp_connect c_bdaddr_any c_bdaddr_local SDPRetryIfBusy 
        throwErrnoIfMinus1_ "sdp_record_register" $ c_sdp_record_register session record 0
